...
| `serve_core` in a test process leaks tokio tasks if the harness is forgotten | Low | Med | `TestDaemon::shutdown(self)` **consumes `self`** — the only path to clean teardown is explicit. `#[must_use]` on the struct surfaces a compiler warning when a test drops it without calling shutdown. **No `Drop` impl.** `block_on` from inside a `#[tokio::test]` panics with "cannot start a runtime from within a runtime"; `abort()` inside `Drop` during a panicking unwind (the exact case where shutdown is most likely to be skipped) triggers a double-panic and crashes the whole test runner instead of failing one test. Relying on `#[must_use]` alone is the correct trade — surface the warning at compile time, do nothing at runtime. |
