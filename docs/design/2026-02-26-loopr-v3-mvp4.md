...
MVP1 proved the orchestration spine: daemon-as-single-authority, FSMs, TaskStore, IPC, worktrees, Ticks. MVP2 added read-only LLM intelligence via the Doc Validator. MVP3 added the Implementer and Reviewer agents, completing the code-level RWL: WorkItem → code in worktree → Bundle → review → Tick.

But the upper levels of the pipeline remain entirely human-driven. A human must manually create Plans, Specs, Phases, and WorkItems through the TUI. The "dev team in a box" has two team members (Implementer, Reviewer) and a paperwork checker (Validator). The Coordinator, Architect, Researcher, and Integrator seats are empty.
...
|-------|-------|-------|-------------|-------------|----------------|
| **Coordinator** | All | Thinking | CreatePlan, CreateSpec, CreatePhase, CreateWorkItem, AssignAgent, SpawnResearcher, AcquireLock, ReleaseLock, ValidateDocument, TriageBundle, AcceptBundle, Transition, NeedHelp, Done | 0.2 | ∞ (long-lived) |
...
1. Load global state from TaskStore
   ...
   - WorkItems (with status and assignees)
...
2. Assess: what level needs attention?
   ...
   - Active Phases, no WorkItems? → create WorkItems
   - Ready WorkItems, Implementer pool not full? → acquire locks on resource_tags, spawn Implementers
   - Proposed/Triaged Bundles? → triage and route to Reviewers
   - Reviewed Bundles? → accept (transition Reviewed → Accepted)
   - All WorkItems Done in an Active Phase? → mark Phase Complete
...
**Status preconditions:** Hierarchy transitions are gated by both child completion AND current status. "All WorkItems Done in a Phase" only triggers `Phase → Complete` if the Phase is currently `Active`. The FSM rejects invalid transitions, but the Coordinator avoids wasted iterations by checking status preconditions in the prompt context.
...
Callable from the TUI (keybinding `g`) or CLI (`loopr coordinator set-goal "Build feature X"`). The goal is persisted in TaskStore as a singleton `CoordinatorGoal` record (implements `Record` trait). This preserves the MVP1 invariant that all meaningful state is in TaskStore and survives daemon crashes. When the Coordinator sees no active Plan and a goal record exists, it generates a Plan from the goal. If no goal exists, the Coordinator operates on existing hierarchy only (managing WorkItems, Bundles, etc.) and does not generate new Plans.
...
| **WorkItem** | All WorkItems for a Phase created; each is small enough for ~5-10 Implementer iterations | No Doc Validator gate (WorkItems are operational, not documents). Coordinator judgment. |
...
| **WorkItem Generation** | Active Phase, relevant Learnings, codebase findings | WorkItem records (title, description, resource tags, dependencies) |
...
The Researcher's output is always a Learning (scoped to the relevant Plan/Spec/Phase/WorkItem). The Coordinator picks up these Learnings on its next iteration.
...
**Coordinator grants locks:** When the Coordinator assigns an Implementer to a WorkItem, it acquires locks on the WorkItem's `resource_tags` (file paths, module names). The Coordinator is the `granted_by` authority; the WorkItem ID is the `holder_id`.

**Executor checks locks on WriteFile:** When `ConflictPolicy::LockStrict` is active, `execute_action()` for `WriteFile` checks the Stores for active Locks on the target file path. If the path is locked by a different `holder_id` than the current agent's `work_item_id`, the write is rejected with `ActionResult::ActionError("file locked by work_item X")`. The Implementer can then `NeedHelp` or work on a different file. With `ConflictPolicy::LockAdvisory`, the check is skipped and conflicts are detected at Bundle merge time by the Integrator.

**Coordinator releases locks:** When a WorkItem reaches `Done` or `Abandoned`, the Coordinator releases its locks.
...
**Lock granularity:** Locks are on individual file paths, not directories. A WorkItem with `resource_tags: ["src/agents/mod.rs", "src/agents/coordinator.rs"]` acquires two locks. If the Coordinator needs to lock an entire directory, it acquires locks on each file the WorkItem will touch (based on resource_tags).
...
**Proposer vs. direct generation:** For simple Plans/WorkItems, the Coordinator generates directly (one LLM call). For complex Specs and Phase breakdowns, spawning 2-3 Proposers produces higher quality through diversity. The Coordinator prompt includes guidance: "For Specs with significant design decisions, spawn 2-3 proposers rather than generating directly."
...
The Coordinator uses iteration records to track progress: "Implementer on WorkItem X completed iteration 5, wrote foo.rs, tests pass, next is error handling." This informs Coordinator decisions about whether to wait, reassign, or mark done.
...
**Iteration record lifecycle:** Iteration Learnings are ephemeral. When a WorkItem reaches `Done` or `Abandoned`, the Coordinator releases locks AND archives iteration records for that WorkItem (transition to a low-confidence state so they fall out of `select_learnings` results). This prevents unbounded growth. Only the most recent iteration record per agent session is kept at full confidence; older ones are superseded.
...
pub struct AgentSession {
    // ... existing fields (id, agent_type, work_item_id, bundle_id, status,
...
- `Implementer`: requires `work_item_id`
...
pub enum AgentAction {
    ...
    CreateWorkItem { phase_id: String, title: String, description: String },
...
3. Creates `AgentSession` with appropriate target field (`work_item_id` for Implementer, `bundle_id` for Reviewer)
...
- `resource_tags`: derived from the WorkItem's `resource_tags` if the agent has a `work_item_id`
...
- For structured lists (WorkItems, Learnings, agent sessions): drop items from the end (lowest priority) until under budget. Never truncate an item mid-text.
...
- **Implementer:** `[(work_item_id, WorkItem), (phase_id, Phase), (spec_id, Spec), (plan_id, Plan)]` — the full chain from WorkItem up to Plan, traversed during context loading
- **Reviewer:** same chain as Implementer (loaded from Bundle → WorkItem → Phase → Spec → Plan)
...
|------|-----------------|--------------|-----------------|
| **Coordinator** | All active + Draft Plans/Specs/Phases (summary: title, status, id) | WorkItem statuses, Bundle statuses, Tick status, active agent sessions (type, status, target, query) | Process learnings, ≥0.6 confidence |
...
| **Implementer** | Full chain: Plan → Spec → Phase → WorkItem (titles + descriptions) | Git worktree state (diff --stat, log) | All scoped learnings ≥0.3 confidence |
| **Reviewer** | Full chain: Plan → Spec → Phase → WorkItem + Bundle | Bundle diff, touched paths | Module-scoped learnings ≥0.3 confidence |
| **Integrator** | None (operates on Bundles/Ticks directly) | Accepted Bundles (id, work_item_id, base_tick_id, touched_paths), current Tick, recent failures | Failure + integration learnings ≥0.3 confidence |
...
4. `create_work_item` — Create a WorkItem under a Phase (Draft)
   {"action": "create_work_item", "phase_id": "...", "title": "...", "description": "..."}
5. `assign_agent` — Start an Implementer or Reviewer on a target
   {"action": "assign_agent", "agent_type": "implementer", "target_id": "work-item-id"}
...
7. `acquire_lock` — Lock a resource before assigning work
   {"action": "acquire_lock", "resource": "src/agents/mod.rs", "holder_id": "work-item-id"}
...
- Create WorkItems that are small enough to fit in half a context window.
- Don't assign more agents than the pool_size allows (check active sessions).
- Acquire locks on resource_tags BEFORE assigning Implementers. Release locks when WorkItems complete.
...
#### WorkItem Generation Prompt (user message, level-specific)
...
## Existing WorkItems in this Phase

{list of existing WorkItems with IDs and statuses, or "None yet"}
...
Create WorkItems for this Phase. Each WorkItem should:
...
- Identify dependencies on other WorkItems

Respond with a JSON array of `create_work_item` actions with `phase_id` set to "{phase.id}".
```
...
| **Bundle Size** | `max_files_touched: u32` (default 8), `max_loc_changed: u32` (default 300) | See defaults | Implementer prompt guidance + Coordinator WorkItem sizing |
...
Most agent actions map to existing IPC methods (plan.create, spec.create, work_item.create, agent.start, bundle.transition, tick.create, etc.) via the `AgentIpcBridge`. The Coordinator, Researcher, and Integrator do NOT need custom IPC methods — they use the same `dispatch()` as everyone else.
...
- AcquireLock / ReleaseLock (acquire on resource_tags, release on WorkItem completion)
- Lock conflict: Coordinator rejects assignment when resource already locked by another WorkItem
...
**Files:**
- `src/agents/generation.rs` (NEW) — `PlanGenerationPrompt`, `SpecGenerationPrompt`, `PhaseGenerationPrompt`, `WorkItemGenerationPrompt`, `build_generation_message()`
...
- WorkItem generation prompt includes existing WorkItems (avoids duplicates)
...
- Integration test: full Plan → Spec → Phase → WorkItem generation pipeline with mock LLM
...
- Full pipeline: human sets goal → Coordinator creates Plan → validates → generates Spec → validates → generates Phases → validates → creates WorkItems → assigns Implementer → Implementer proposes Bundle → Coordinator triages + routes to Reviewer → Reviewer approves → Coordinator accepts → Integrator creates Tick → publishes → Coordinator marks Phase complete
...
- Learning propagation: Implementer creates Learning → Coordinator picks it up → influences next WorkItem
...
**Lock ordering invariant (for handlers):** Code paths that must acquire multiple locks follow this order: `plans → specs → phases → work_items → bundles → ticks → learnings → locks → agent_sessions → Store mutex`. Handlers acquire at most one write lock at a time, release it, then acquire the Store mutex for persistence.
...
- Full pipeline: Plan → Spec → Phase → WorkItem → Bundle → Tick
