...
**Amended 2026-05-09 (post-implementation, Architect Round 2):** the original phase assumed the reviewer prompt enumerated per-AC results that a parser could extract. Verification against `crates/context/prompts/agents/reviewer/system.pmt` showed the prompt restricts the LLM to a single `Verdict` JSON object (`accept` / `change_requested` / `reject`) — there is no per-AC array on the wire to parse. Under the phase's `events-only, no domain change` constraint, the only consistent path is **synthesis from the parsed Verdict** plus the Work's `acceptance_criteria`, not parsing. The bullets below reflect what the implementation actually does and what the design now sanctions; the original parser/partial-warn/total-parse-failure language has been struck.

- `agents::reviewer::emit_ac_events` synthesizes a `Vec<AcResult>` from the parsed `domain::Verdict` plus the Work's `acceptance_criteria`. Status mapping: `Accept` → every AC `verified`; `ChangeRequested` → ACs whose text appears (whole-word, length ≥ 4) in any `ChangeRequested.reasons[*].message` map to `failed` with `location: message` evidence, the rest to `not_applicable`; `Reject` → every AC `not_applicable`.
- For each AC, the reviewer emits a `debug!(target: "reviewer.ac", criterion, status, evidence)` event.
- `ReviewerDeps` gains a `path_deny_patterns: Vec<String>` field (mirroring the implementer dispatcher's), wired through `ContextBuilder` setup at daemon startup. The reviewer applies `telemetry::transcript::redact_paths(evidence, &deps.path_deny_patterns)` before emitting the event so caller-supplied deny patterns hit the evidence field. This is the only Reviewer-side dep change in this doc.
- A roll-up event lands after the per-AC events: `info!("reviewer: ac roll-up", ac_count, ac_verified, ac_skipped, ac_failed)`.
- ~~Partial extraction~~ / ~~total parse failure~~ branches are **inapplicable** to synthesis: synthesis always produces N AcResults for N ACs. If the Work has zero ACs declared, no per-AC events fire and no roll-up is emitted (the boolean accept/reject transition still happens regardless).
- `domain::Verdict` is **not modified**; per-AC data is observability, not a record.
- The Bundle's `summary.md` renderer is **not modified** by this phase. Tracked as a follow-up: `docs/design/<future>-verdict-ac-results-domain-promotion.md`. A future doc that widens the prompt to require structured per-AC verification AND promotes `Vec<AcResult>` into the domain Verdict would replace synthesis with a real parser; that work is out of scope here.
- Snapshot test asserts a happy-path review emits `reviewer.ac` events plus the four roll-up counts (see `crates/agents/tests/reviewer_ac_visibility.rs`).
...
**Amended 2026-05-09 (post-implementation, Architect Round 2):** the original phase asked for both `director.accept_bundle` and `director.reject_bundle` spans. Verification against `crates/agents/src/director.rs` (`DirectorAction` enum + `WorkSpawner` trait) showed the Director has no `RejectBundle` action and no `WorkSpawner::reject_bundle` method — Bundle rejections are not a Director decision in v5. They are sourced from (1) the Reviewer's `Verdict::Reject` / `Verdict::ChangeRequested` transitioning Bundle → `BundleStatus::Rejected` inside `agents::reviewer::run_reviewer` (already covered by Phase 5's `reviewer.*` events) and (2) the Integrator's `IntegrationError` transitioning Bundle → `BundleStatus::IntegrationFailed` inside `integrator::fail_all_without_reset` (already covered by Phase 3's `integrator.fail_all` events). The `director.reject_bundle` requirement is struck below; if a future doc widens `DirectorAction` with a `RejectBundle` variant (a Director-behavior change, not an observability sweep), the span lands then.

- Add `director.accept_bundle` `#[instrument]` function in `agents::director`. The accept path gates the Bundle Reviewed → Integrating transition.
- ~~`director.reject_bundle`~~: removed; see amendment above. Bundle rejections are observed via Phase 5's `reviewer.*` events and Phase 3's `integrator.fail_all` events.
...
- Snapshot test asserts the accept span emits on the appropriate path (`crates/agents/tests/director_visibility.rs`).
...
- [x] **Phase 5 implementation drift (resolved 2026-05-09, Architect Round 2):** Architect accepted that the prompt restricts the LLM to a `Verdict` JSON object with no per-AC array, so a parser had nothing to parse. Phase 5 amended above to sanction synthesis from `Verdict` + `Work.acceptance_criteria` as the canonical path. A future doc may widen the prompt and promote `Vec<AcResult>` into the domain Verdict; that work is out of scope here.
- [x] **Phase 7 implementation drift (resolved 2026-05-09, Architect Round 2):** Architect accepted that `DirectorAction` has no `RejectBundle` variant in v5; Bundle rejections are sourced from the Reviewer (Phase 5 events) and the Integrator (Phase 3 events). Phase 7 amended above to remove `director.reject_bundle`. If a future doc widens `DirectorAction`, the span lands then.
- [ ] **Phase 8 deferred fields (post-implementation, 2026-05-09):** the Work summary lacks `total_iterations`, `lifeguard_fires`, and `director_override_count` — these are agent-runtime counters local to `run_implementer`'s loop and the Director's spawner state, not store-resident. Surfacing them would require either domain changes (add iteration/lifeguard counters to `Bundle` or `Work`) or threading counts through `transition_and_persist_work`'s 17+ call sites. The Plan summary lacks `total_input_tokens`, `total_output_tokens`, `total_cost_usd` — these live on `MeteredLlmClient`'s `ProcessSnapshot` and would require threading a snapshot handle into `transition_and_persist_plan` (or splitting summary emission off from the FSM helper). The 2026-05-09 follow-up commit (ac879be) added `ticks` / `bundles_accepted` / `bundles_rejected` from store queries; the remaining five fields are tractable in a follow-up.
