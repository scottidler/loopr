...
| **WorkItem** | A concrete, assignable unit of work within a Phase. The thing an Implementer actually does. |
| **Plan → Spec → Phase → WorkItem** | The work decomposition hierarchy. Plan is the goal, Spec is the detailed design, Phase is a sequential implementation step, WorkItem is an atomic task. |
...
- Prove the WorkItem → Bundle → Tick pipeline works end-to-end with human-driven state transitions
- Validate three FSMs (WorkItem, Bundle, Tick) with role-based guards and per-state invariants
...
**WorkItem** — Concrete unit of work within a Phase. Has a full FSM.
...
pub struct WorkItem {
    ...
    pub dependencies: Vec<String>,  // IDs of prerequisite WorkItems
    pub created_at: i64,
    pub updated_at: i64,
}
...
pub struct Bundle {
    pub id: String,
    pub work_item_id: String,       // Which WorkItem this fulfills
...
pub struct Learning {
    ...
    pub scope: LearningScope,       // WorkItem | Phase | Spec | Plan | Global
...
| WorkItem | `work_items` | `phase_id`, `status`, `assignee` |
| Bundle | `bundles` | `work_item_id`, `status`, `base_tick_id` |
...
```rust
const WORK_ITEM_TRANSITIONS: &[TransitionRule<WorkItemStatus>] = &[
...
3. InReview implies exactly one active Bundle for this WorkItem
4. Dependencies must be acyclic (validated on create/update, not on transition)
5. WorkItem must have non-empty resource_tags before transitioning to Ready+
...
- `work_item.create`, `work_item.get`, `work_item.list`, `work_item.update`
...
- `work_item.transition { id, target_status, role }`
- `bundle.transition { id, target_status, role }`
- `tick.transition { id, target_status, role }`

**Worktree Operations:**
- `worktree.create { work_item_id, base_ref }`
- `worktree.refresh { work_item_id }` — rebase worktree onto latest Published Tick's SHA (required to clear staleness)
- `worktree.list`
- `worktree.cleanup { work_item_id }`
...
- `bundle.rejected_stale { bundle_work_item_id, base_tick_id, latest_tick_id }` — bundle proposal rejected due to staleness guard
- `worktree.created { work_item_id, path }`
- `worktree.cleaned { work_item_id }`
...
```rust
async fn handle_work_item_transition(
    ...
    let mut item: WorkItem = ctx.store.get(id)?
    ...
    let _ = ctx.event_tx.send(DaemonEvent::transition_completed(
        "work_items", id, from, target, role,
    ));
...
loopr work-item create --phase <id> --title "..." --tags src/foo.rs,Cargo.toml
                                              # → work_item.create
```
...
loopr work-item transition <id> <status>      # → work_item.transition { id, target_status, role }
...
```
loopr worktree create <work-item-id>          # → worktree.create { work_item_id, base_ref: latest tick SHA }
loopr worktree refresh <work-item-id>         # → worktree.refresh { work_item_id } (rebase onto latest tick)
loopr worktree list                           # → worktree.list
loopr worktree cleanup <work-item-id>         # → worktree.cleanup { work_item_id }
```

**Bundle Operations (as Implementer):**
```
loopr bundle propose <work-item-id> --notes "..."
...
impl WorktreeManager {
    ...
    pub fn create(&self, work_item_id: &str, base_ref: &str) -> Result<PathBuf> {
    ...
    pub fn refresh(&self, work_item_id: &str, new_base_ref: &str) -> Result<()> {
        let path = self.worktree_dir.join(work_item_id);
    ...
    pub fn cleanup(&self, work_item_id: &str) -> Result<()> {
        let path = self.worktree_dir.join(work_item_id);
        Command::new("git")
...
- End-to-end: Plan → Spec → Phase → WorkItem → worktree → Bundle → Tick
...
- Crash recovery (detect orphaned InProgress WorkItems and Integrating Bundles on daemon startup)
...
4. WORK ITEM (as Coordinator)
   └─ Create WorkItem "Implement JWT signing" under Phase 1
...
6. BUNDLE (as Implementer)
   ...
   └─ Transition WorkItem InProgress → InReview
...
9. COMPLETION (as Coordinator)
   └─ Transition WorkItem Integrated → Done
   └─ Clean up worktree
   └─ Move to next WorkItem

10. STALENESS GUARD (proof point — do this on a second WorkItem)
   └─ Start a second WorkItem, create its worktree, make changes, commit
...
- **Request** (client → daemon): has `"method"` field → `{ "id": 1, "method": "work_item.get", "params": {...} }`
...
- End-to-end pipeline: Plan → Spec → Phase → WorkItem → Bundle → Tick
...
1. **Orphaned worktrees** — List `.worktrees/` entries. For each, check if the associated WorkItem is in InProgress state. If the WorkItem doesn't exist or is in a terminal state (Done, Abandoned), clean up the worktree and delete the branch.
2. **InProgress WorkItems without worktrees** — If a WorkItem is InProgress but its worktree is missing, transition it back to Ready (Coordinator role, logged as recovery action).
...
- `WorkItem.phase_id` must reference an existing Phase
- `Bundle.work_item_id` must reference an existing WorkItem
- `WorkItem.dependencies` must all reference existing WorkItems
...
- To remove a hierarchy, delete bottom-up (WorkItems first, then Phases, then Specs, then Plan)
...
If `agent/<work_item_id>` branch already exists (from a previous failed attempt):
