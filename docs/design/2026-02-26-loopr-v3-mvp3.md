...
| **Staleness Cascade** | When a Tick publishes, all in-progress WorkItems with an older `base_tick_id` are automatically notified. Their Bundles are marked stale. Agents must refresh their worktree and rebase before resubmitting. |
...
- **No automated implementation.** WorkItems sit in `Ready` until a human manually creates code in a worktree, commits it, and proposes a Bundle. The whole point of Loopr is to automate this.
...
- **Manual staleness handling.** When a Tick publishes, in-progress WorkItems with older `base_tick_id` are stale, but no one is automatically notified. Agents would re-submit stale Bundles without realizing.

### Goals

- **G1:** LLM Implementer agents autonomously take WorkItems from `InProgress`, work in worktrees, produce code, run tools, and propose Bundles
...
│                        TUI / CLI                                  │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌────────────────┐   │
│  │Dashboard │  │WorkItems │  │ Bundles  │  │  Agent View    │   │
│  │          │  │          │  │          │  │ (new in MVP3)  │   │
│  │          │  │          │  │          │  │ • live output  │   │
│  │          │  │          │  │          │  │ • tool results │   │
│  │          │  │          │  │          │  │ • agent status │   │
...
pub struct AgentSession {
    ...
    pub work_item_id: Option<String>,  // what it's working on (Implementer)
...
Here's what happens when an Implementer agent processes iteration #3 for WorkItem "Add error handling to the parser":

1. **Load context**: Query TaskStore for the full hierarchy — Plan "Improve parser robustness", Spec "Error recovery design", Phase 2 "Implement error types", WorkItem "Add error handling". Query Learnings scoped to this WorkItem. Run `git diff --stat` and `git log --oneline -5` in the worktree to see current state.

2. **Construct prompt**: Assemble ~4K tokens of context:
   - System prompt (Implementer role, capabilities, rules)
   - Hierarchy summary (Plan → Spec → Phase → WorkItem descriptions)
...
Each Implementer agent is assigned a single WorkItem in `InProgress` state. It works in the WorkItem's worktree.
...
│                                                          │
│  1. Load Context                                         │
│     ├─ Plan → Spec → Phase → WorkItem (full hierarchy)   │
│     ├─ Relevant Learnings (scoped to this WorkItem)      │
...
Each Reviewer agent is assigned a Bundle in `Triaged` state. It computes the diff from the Bundle's worktree using `git diff <base_tick_sha>..<head>` and reads the related WorkItem/Phase/Spec context.
...
│     ├─ WorkItem → Phase → Spec → Plan (hierarchy)        │
│     ├─ Relevant Learnings                                │
...
**External state changes:** When a WorkItem's state changes externally (Coordinator transitions it to `Abandoned`, `Blocked`, etc.) while an agent is running:
- The agent checks WorkItem status at the start of each iteration via `AgentIpcBridge`
- If WorkItem is no longer `InProgress`, the agent stops gracefully (Completed or Cancelled depending on the new state)
...
3. For each running Implementer agent:
   - Compare the agent's WorkItem `base_tick_id` with the new Tick number
   - If stale: set a "stale" flag on the agent
   - On next iteration, agent sees the stale flag
   - Agent refreshes worktree: `worktree_mgr.refresh(work_item_id, new_tick_sha)`
   - Agent updates WorkItem's `base_tick_id` to the new Tick
...
if agent_state.stale {
    ...
    worktree_mgr.refresh(&work_item_id, &latest_tick.integration_sha)?;

    // Update WorkItem's base_tick_id
    ipc_bridge.request(DaemonRequest {
        method: "work_item.update_base_tick".into(),
        params: json!({
            "id": work_item_id,
...
| 2 | WorkItem description + acceptance criteria | ~200 tokens | 1K tokens |
...
to implement a specific WorkItem by writing code in a Git worktree.
...
|--------|--------|---------|------|-------------|
| `agent.start` | `{agent_type, work_item_id?, bundle_id?}` | `AgentSession` | Coordinator | Start an agent for a WorkItem or Bundle |
...
```
loopr agent start implementer <work_item_id>     # Start Implementer agent
...
The `work_item.transition` handler gains awareness of agent sessions:

- When a WorkItem transitions to `InProgress` and `agents.enabled = true`, optionally auto-start an Implementer agent
- When a WorkItem transitions to `Abandoned` or `Done`, stop any running agent for that WorkItem
...
5. Update WorkItem `base_tick_id` after refresh
...
**Integration tests:**
- Full Implementer loop with mock LLM: WorkItem → agent writes file → runs test tool → proposes Bundle
...
1. **Phase 1-3** ship together — Foundation + Implementer + Tools. This gives a working (non-streaming) Implementer that can take a WorkItem and produce a Bundle.
...
|---|-----------|---------------|
| 1 | Implementer agent takes a WorkItem from InProgress and produces a Bundle | `loopr agent start implementer <wi_id>` → agent writes code → `loopr bundle list` shows new Bundle |
...
| Context window overflow (too much hierarchy + diff + tool output) | Medium | Medium | Output truncation. Priority-based context assembly (WorkItem description > Learnings > full hierarchy). Configurable max_tokens per context section. |
...
- [ ] Should agent auto-start be tied to WorkItem/Bundle transitions, or always require explicit `agent.start`?
