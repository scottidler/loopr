...
- **Console layer ignored stricter user filters.** Initial implementation hardcoded the console layer's filter to `EnvFilter::new("info")`, which meant `loopr --log-level warn` still printed `info`-level events to stderr because each layer evaluates its filter independently. **First fix (v0.5.3):** composed the user directive with `LevelFilter::INFO` via `FilterExt::and`, so the console sees events that pass both filters — stricter wins, INFO floor prevents debug/trace spam. **Second fix (follow-up to round 2 audit):** the `FilterExt::and` approach produced a console layer of type `Filtered<..., And<EnvFilter, LevelFilter>, _>`, diverging from the other layers' `Filtered<..., EnvFilter, _>`. Rebuilt the console filter as a plain `EnvFilter` whose directive string is the user's directive with every `trace`/`debug` clause clamped to `info` (new pub(crate) helper `subscriber::floor_at_info`). All four layers now carry the same `EnvFilter` type, simplifying any future runtime-reload path, while the INFO floor semantics are preserved and unit-tested.
...
- **`telemetry::init` directive-type rework (follow-up to round 2).** The spec prescribed `init(target, run_id, filter: EnvFilter)`. The v0.5.3 implementation matched, but relied on a `filter_clone` helper that round-tripped an `EnvFilter` through its `Display` form per layer (because `EnvFilter: !Clone`), swallowing re-parse failures via `unwrap_or_else(|_| EnvFilter::new("info"))` — a silent-fallback hack. **Rework:** `init` now takes `directive: &str`, validates once at the top (before any I/O — a bad directive still cannot leave a ghost run dir behind), and each layer does its own fresh `EnvFilter::try_new(directive).expect("validated by caller")`. The `filter_clone` helper is gone; the silent fallback is gone; `resolve_log_filter` in `loopr` is now `resolve_log_directive` and returns `String` (validation moved to `telemetry::init` to keep a single parse path). A new `TelemetryInitError::InvalidFilter { directive, reason }` variant carries the parse failure.
