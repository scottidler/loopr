...
MVP1 proved the orchestration spine: three FSMs (WorkItem, Bundle, Tick), daemon-mediated correctness, NDJSON IPC over Unix socket, Git worktree management, and a ratatui TUI. All 11 success criteria pass end-to-end. But two gaps remain:
...
- Daemon crash or restart loses all Plans, Specs, Phases, WorkItems, Bundles, Ticks, Learnings, and Locks. This makes loopr unusable for real multi-session work.
...
- **No staleness cascade automation.** When a Tick publishes, in-progress WorkItems are not auto-notified. Human discovers staleness on next bundle proposal (same as MVP1).
...
// Same pattern for Phase (spec_id, status, order), WorkItem (phase_id, status),
// Bundle (work_item_id, status, base_tick_id), Tick (status, number),
...
| 4 | Crash recovery works with persistent state | Create InProgress WorkItem → kill daemon → restart → WorkItem is Blocked |
