...
The scaffolded Rust binary built by `cargo init` has no version reporting.
Users have no way to confirm which version of the tool they are running.
Adding `--version` is the baseline expectation for any CLI tool and is
required before the binary is useful in scripted environments.
...
No contract changes. This plan adds a CLI flag to an existing binary
without defining new data models or public interfaces.
...
- **Add --version to main.rs**: Parse `std::env::args()`. If the first
  argument is `--version`, print `env!("CARGO_PKG_VERSION")` and exit
  with code 0. Otherwise execute the existing body. All changes in
  `src/main.rs`.

- **Add version test**: In `src/main.rs` under `#[cfg(test)]`, add a
  test that builds the binary with `std::process::Command` and asserts
  the output of `--version` contains the string from
