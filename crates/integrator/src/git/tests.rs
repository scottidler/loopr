...
///
/// Also covers Link 5 (`crates/integrator/src/lib.rs:278`): an untracked
/// root `Cargo.lock`, generated by the target's own `cargo` build, is
/// exactly the kind of artifact that must be removed here too, since the
/// pre-guard clean now runs `clean_fd` before the dirty-tree guard
/// evaluates the tree.
#[tokio::test]
async fn clean_fd_preserves_loopr_state_and_removes_artifacts() {
    ...
    std::fs::write(td.path().join("Cargo.lock"), b"# untracked build artifact\n").unwrap();
    ...
    assert!(
        !td.path().join("Cargo.lock").exists(),
        "clean_fd must remove an untracked root Cargo.lock (Link 5 build artifact)"
    );
}
