...
//!
//! Log-capture tests install a thread-local capturing subscriber via
//! `tracing::subscriber::set_default` (or `with_default`) and then assert on a
//! specific `warn!`/`error!` line emitted by the code under test. Without the
//! global default installed by [`ensure_global_interested_default`], those
//! assertions are flaky (`store` measured ~16% over 150 full-binary runs;
//! `llm` ~8% over 200), intermittently seeing zero captured lines.
//!
...
//! # Contract
//!
//! Every log-capturing test in the workspace MUST call
//! [`ensure_global_interested_default`] before installing its thread-local
//! capturing subscriber. [`crate::init_for_test`] calls it internally, so its
//! callers are covered automatically. The one hazard is a test binary that
//! also installs its *own* process-global default (via `set_global_default`,
//! `try_init`, or `SubscriberInitExt::init`): that conflicts with this helper
//! and such a binary must be exempted, never routed through it.
...
///
/// std-only by construction (no `tokio`, no network), so it fits telemetry's
/// "must compile without tokio" rule.
...
/// installing its thread-local capturing subscriber (a per-module
/// `set_capturing_default` helper typically does so). Idempotent and cheap
/// after the first call.
///
/// The result of `set_global_default` is intentionally ignored: another test
/// path in the same binary may already have installed a compatible global
/// default, and that is fine. This is safe only because binaries that install
/// an *incompatible* global default (e.g. an `EnvFilter`-backed one that can
/// cache `Interest::never`) are exempted rather than routed through here.
