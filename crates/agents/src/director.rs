...
// Reconcile sweep (`reconcile_director`) lives in its own submodule
// (mirroring `mode.rs` / `pattern.rs`) to keep this file under the
// 1500-line bloat-task cap; re-exported below so `director::reconcile_director`
...
pub use reconcile::reconcile_director;
