...
- `run_reviewer` -> per-criterion `debug!(target: "reviewer.ac", criterion_id, criterion, status, evidence)` plus `info!("reviewer: ac roll-up", ac_count, ac_met, ac_unmet)` (Phase 8 of `docs/design/2026-07-11-verified-swarm.md` re-keyed these on the criterion's stable `id` — dropping the Phase-5 fuzzy word-matcher — and collapsed status to `met`/`unmet`; `evaluate_and_emit_criteria` returns the `Vec<CriterionResult>` that Phase 11 now persists onto the `Review` record). `run_reviewer` also emits `info!("reviewer: Review record persisted", review_id, round, verdict)` (Phase 11) after appending one `Review` row per round through the `store::ReviewSink`.
